...
public class JobController {
    ...
    @PostMapping(ID + "/finish-job")
    public ResponseDto<JobResponseDto> finishJob(
            @AuthenticationPrincipal JwtPrincipal jwt, @PathVariable UUID jobId) {
        JobResponseDto response = jobUserService.reportSuccessfulEndJob(jwt.id(), jobId);
        return new ResponseDto<>(
    ...
    @PatchMapping(ID + "/report-problem-true")
    public ResponseDto<JobDispatcherResponseDto> reportProblemTrue(
            @AuthenticationPrincipal JwtPrincipal jwt, @PathVariable UUID jobId) {
        JobDispatcherResponseDto response = jobUserService.reportProblemTrue(jwt.id(), jobId);
        return new ResponseDto<>(
                SuccessCode.RESOURCE_UPDATED,
    ...
    @PatchMapping(ID + "/report-problem-false")
    public ResponseDto<JobDispatcherResponseDto> reportProblemFalse(
            @AuthenticationPrincipal JwtPrincipal jwt, @PathVariable UUID jobId) {
        JobDispatcherResponseDto response = jobUserService.reportProblemFalse(jwt.id(), jobId);
        return new ResponseDto<>(
                SuccessCode.RESOURCE_UPDATED,
